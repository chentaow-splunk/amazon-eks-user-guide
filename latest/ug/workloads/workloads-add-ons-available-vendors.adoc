...
For information about the add-on, see https://help.splunk.com/en/splunk-observability-cloud/manage-data/splunk-distributio[…]etry-collector/collector-for-kubernetes/kubernetes-eks-add-on[Install the Splunk add-on for Amazon EKS] in the Splunk documentation.
